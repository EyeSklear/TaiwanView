...
var __exports__ = (() => {
  ...
  var require_pbf = __commonJS({
    "../../node_modules/pbf/index.js"(exports, module) {
      ...
      var SHIFT_RIGHT_32 = 1 / SHIFT_LEFT_322;
      ...
      Pbf.prototype = {
        ...
        readFixed64: function() {
          var val = readUInt32(this.buf, this.pos) + readUInt32(this.buf, this.pos + 4) * SHIFT_LEFT_322;
          this.pos += 8;
          return val;
        },
        readSFixed64: function() {
          var val = readUInt32(this.buf, this.pos) + readInt32(this.buf, this.pos + 4) * SHIFT_LEFT_322;
          this.pos += 8;
          return val;
        },
        readFloat: function() {
...
${formatErrorKey(key)}: ${errorContext[key]}`;
  ...
  var PropertiesReader = class {
    ...
    static _readField(tag, obj, pbf) {
      if (tag === 1)
        obj.data.push(KeyValueObjectReader.read(pbf, pbf.readVarint() + pbf.pos));
    }
  };
  var DoublesReader = class {
    ...
    static _readField(tag, obj, pbf) {
      if (tag === 1)
        pbf.readPackedDouble(obj.value);
      else if (tag === 2)
  ...
  function parsePbf(buffer, TileReader3) {
    const pbf = new import_pbf.default(buffer);
    const tile = TileReader3.read(pbf);
    return tile;
  }
  ...
  var IndicesReader = class {
    ...
    static _readField(tag, obj, pbf) {
      if (tag === 1)
        readPackedFixed64(pbf, obj.value);
    }
  };
  ...
  var SHIFT_LEFT_32 = BigInt((1 << 16) * (1 << 16));
  ...
  function readPackedFixed64(pbf, arr) {
    if (pbf.type !== 2)
      return arr.push(readFixed64(pbf));
    const end = readPackedEnd(pbf);
    arr = arr || [];
    while (pbf.pos < end)
      arr.push(readFixed64(pbf));
    return arr;
  }
  ...
  (0, import_core2.registerLoaders)([carto_spatial_tile_loader_default]);
  ...
  var SpatialIndexTileLayer = class extends import_geo_layers3.TileLayer {
    getTileData(tile) {
      ...
      tile.url = typeof data === "string" || Array.isArray(data) ? (0, import_geo_layers3._getURLFromTemplate)(data, tile) : null;
      if (!tile.url) {
        return Promise.reject("Invalid URL");
      }
      if (getTileData) {
  ...
  function quadbinToWorldBounds(quadbin, coverage) {
    ...
    const mask = 1 << z;
    const scale = mask / TILE_SIZE2;
    return [[x / scale, TILE_SIZE2 - y / scale], [(x + coverage) / scale, TILE_SIZE2 - (y + coverage) / scale]];
  }
  ...
  function getLayer(type, config, dataset) {
    ...
    const layerTypeDefs = {
      point: {
        Layer: import_layers2.GeoJsonLayer,
        propMap: {
          columns: {
            altitude: (x) => ({
              parameters: {
  ...
  function layerFromTileDataset(formatTiles = TILE_FORMATS.MVT, scheme) {
    if (scheme === "h3") {
      return H3TileLayer;
    }
    if (scheme === "quadbin") {
  ...
  function getTextPixelOffsetAccessor({
    alignment,
    anchor,
    size
  }, radius) {
    ...
    return {
      ...
      layers: extractTextLayers(layers.reverse()).map(({
        ...
        try {
          ...
          import_core6.log.error(e.message)();
          return void 0;
        }
      })
    };
  ...
  function extractTextLayers(layers) {
    return (0, import_core6._flatten)(layers.map(({
      id,
      config,
      ...rest
    }) => {
  ...
  async function _fetchDataUrl({
    ...
    if (format2 === FORMATS.TILEJSON && formatTiles) {
      import_core7.log.assert(Object.values(TILE_FORMATS).includes(formatTiles), `Invalid value for formatTiles: ${formatTiles}. Use value from TILE_FORMATS`);
  ...
  var CartoLayer = class extends import_core8.CompositeLayer {
    initializeState() {
      this.state = {
        data: null,
        apiVersion: null
    ...
    _checkProps(props) {
      ...
      import_core8.log.assert(Object.values(API_VERSIONS).includes(apiVersion), `Invalid apiVersion ${apiVersion}. Use API_VERSIONS enum.`);
      if (apiVersion === API_VERSIONS.V1 || apiVersion === API_VERSIONS.V2) {
        import_core8.log.assert(type === MAP_TYPES.QUERY || type === MAP_TYPES.TILESET, `Invalid type ${type}. Use type MAP_TYPES.QUERY or MAP_TYPES.TILESET for apiVersion ${apiVersion}`);
        import_core8.log.assert(!connection, `Connection prop is not supported for apiVersion ${apiVersion}`);
        import_core8.log.assert(!geoColumn, `geoColumn prop is not supported for apiVersion ${apiVersion}`);
        import_core8.log.assert(!columns, `columns prop is not supported for apiVersion ${apiVersion}`);
      } else if (apiVersion === API_VERSIONS.V3) {
        import_core8.log.assert(connection, "Missing mandatory connection parameter");
        import_core8.log.assert(Object.values(MAP_TYPES).includes(type), `Invalid type ${type}. Use MAP_TYPES enum.`);
        if (!COLUMNS_SUPPORT.includes(type)) {
          import_core8.log.assert(!columns, `columns prop is only supported for types: ${COLUMNS_SUPPORT.join()}`);
        }
        if (!GEO_COLUMN_SUPPORT.includes(type)) {
          import_core8.log.assert(!geoColumn, `geoColumn prop is only supported for types: ${GEO_COLUMN_SUPPORT.join()}`);
        }
        if (columns) {
          import_core8.log.assert(Array.isArray(columns), "columns prop must be an Array");
        }
      }
    }
    updateState({
  ...
  __publicField(CartoLayer, "defaultProps", defaultProps7);
  ...
  if (!import_layers3.GeoJsonLayer) {
